...
const Item = ({ ifExists, id, color, shadow, title, text, img, btn, rating, price }) => {
  ...
  return (
    <>
      <div className={`relative bg-gradient-to-b ${color} ${shadow} grid items-center ${ifExists ? "justify-items-start" : "justify-center"} rounded-xl py-4 px-5 transition-all duration-700 ease-in-out w-full hover:scale-105`}>
        ...
        <div className={`grid items-center ${ifExists ? "justify-items-start" : "justify-center"}`}>
          ...
          <div className='flex items-center gap-3'>
            ...
            <button type='button' className='bg-white/90 blur-effect-theme button-theme px-2 py-1 p-0.5 shadow shadow-sky-200 text-sm text-black' onClick={() => {onAddtoCart(); onCartToggle()}}>{btn}</button>
          </div>
        </div>

        <div className={`flex items-center ${ifExists ? "absolute top-5 right-1" : "justify-center"}`}>
          <img src={img} alt={`img-${id}`} className={`transitions-theme hover:-rotate-12 ${ifExists ? "h-auto w-64 lg:w-56 md:w-48 -rotate-[35deg]" : "h-36 w-64"}}`} />
        </div>
      </div>
    </>
  );
